...
void main() {
  ...
  group('Large Dataset Performance & Pagination Tests', () {
    ...
    test('Data structure overhead for 500,000 rows', () async {
      ...
      final List<String> columns = ['id', 'name', 'timestamp', 'value', 'category'];
      ...
      debugPrint('Generated $totalRows rows in ${stopwatch.elapsedMilliseconds} ms');
    });

    testWidgets('Unpaginated rendering performance degrades with large row counts', (WidgetTester tester) async {
      const int rowCount = 5000; // Testing 5,000 rows unpaginated (500k would freeze unit test runner)
              ...
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  rows: rows.map((r) => DataRow(
                    cells: columns.map((c) => DataCell(Text(r[c] ?? ''))).toList(),
                  )).toList(),
                ),
              ),
            ),
      ...
      debugPrint('Unpaginated rendering of $rowCount rows took ${renderStopwatch.elapsedMilliseconds} ms');
    ...
    testWidgets('Paginated / Virtualized rendering of 500,000 rows builds in under 50ms', (WidgetTester tester) async {
      const int totalRows = 500000;
      ...
      List<Map<String, String>> getPage(int pageIndex) {
        ...
        return List.generate(
          pageSize,
          (i) => {'id': '${start + i}', 'name': 'Item ${start + i}', 'status': 'Active'},
        );
      }
      ...
      debugPrint('Paginated view of page 1 ($pageSize items out of $totalRows total) built in ${paginatedStopwatch.elapsedMilliseconds} ms');
