...
void main() {
  ...
  group('BigQueryService Unit Tests', () {
    ...
    test(
      'Initial state loads mock data correctly when Test User is active',
      () {
        ...
        expect(
          service.datasetTables['analytics_dataset'],
    ...
    test(
      'Selecting a new project resets schemas and updates active selection',
      () async {
    ...
    test(
      'estimateQueryCost mock outputs expected byte scanning size',
      () async {
    ...
    test('runQuery mock maps columns and rows correctly', () async {
      ...
      expect(
        service.resultColumns,
        containsAll(['Name', 'Employees', 'Earnings']),
      );
    ...
    test('runQuickCount mock retrieves row count values', () async {
      await service.runQuickCount(
        'SELECT * FROM ecommerce_dataset.transactions',
